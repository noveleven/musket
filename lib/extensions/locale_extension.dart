...
extension LocaleExtension on Locale? {
  bool isCompatible(Locale target,
      {bool script = false, bool country = false}) {
    if (target == this) {
      return true;
    }
    if (target == null) {
...
Locale? findCompatibleLocale(List<Locale?> supportLocales, Locale target) {
  return supportLocales?.firstWhere((locale) => locale == target, orElse: () {
    return supportLocales.firstWhere(
        (locale) => locale.isCompatible(target, script: true), orElse: () {
      return supportLocales.firstWhere(
          (locale) => locale.isCompatible(target, country: true), orElse: () {
        return supportLocales.firstWhere(
            (locale) => locale.isCompatible(target),
            orElse: () => null);
      });
    });
  });
